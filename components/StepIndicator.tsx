...
const StepIndicator: React.FC<StepIndicatorProps> = ({ currentStep }) => {
  return (
    <div className="w-full mb-6 md:mb-8 overflow-x-auto pb-2">
      <div className="flex items-center justify-start md:justify-center min-w-max px-2">
          ...
          return (
            <div key={step.id} className="flex items-center">
              ...
              {index < steps.length - 1 && (
                <div className={`w-4 md:w-8 h-0.5 mx-2 md:mx-4 ${isCompleted ? 'bg-green-500' : 'bg-gray-700'}`} />
              )}
            </div>
          );
        })}
